<?php

// dbconnect.phpに接続
require ('../../dbconnect.php');

// inputタグの中身をsqlに反映させる構文
// HTTPリクエストがPOSTであるかを確認し、POSTならばデータベースへの操作を行います。
if ($_SERVER['REQUEST_METHOD'] === 'POST') {

    // 下3行のコードで画像をディレクトリ内のimgファイルの中にアップロードする。
    // １.HTMLから画像を持ってきて、それにランダムで画像名を付ける。
    // 2.その画像に指定した相対パスを生成
    // 3.画像をアップロードする。
    $image_name = uniqid(mt_rand(), true) . '.' . substr(strrchr($_FILES['image']['name'], '.'), 1);
    $image_path = '../../assets/img/quiz' . $image_name;
    print_r($image_path);
    move_uploaded_file($_FILES['image']['tmp_name'], $image_path);

    // prepareで不完全なSQL文を指定（questionsテーブルに新しい行を挿入）
    $stmt = $dbh->prepare("INSERT INTO questions(content, image, supplement) VALUES(:content, :image, :supplement)");
    // executeで上記のprepareで指定したSQL文を補完して実行
    $stmt->execute([
        "content" => $_POST["content"],
        //sqlの中身は、画像でなく画像名なので、画像名を代入する。
        "image" => $image_name,    
        "supplement" => $_POST["supplement"]
    ]);

    // 最後に挿入された行のIDを取得
        $lastInsertId = $dbh->lastInsertId();
        
    // choicesテーブルに新しい行を挿入するSQL文
    $stmt = $dbh->prepare("INSERT INTO choices(name, valid, question_id) VALUES(:name, :valid, :question_id)");
    for ($i = 0; $i < count($_POST["choices"]); $i++) {
    // choicesテーブルに行を挿入するSQL文を補完して実行
    print_r($_POST["correctChoice"]);
    $stmt->execute([
        "name" => $_POST["choices"][$i],
        // ループで扱っている選択肢と正解の選択肢が等しいとvalidを１に、そうでないと０にする
        "valid" => (int)$_POST['correctChoice'] === $i + 1 ? 1 : 0,
        // question_idに最後にこのコードの前の部分でquestionsテーブルに挿入された質問のIDを代入する
        // PDO::lastInsertId — 最後に挿入された行の ID あるいはシーケンスの値を返す
        "question_id" => $lastInsertId
    ]);
}
}
?>

<!DOCTYPE html>
<html lang="ja">

<head>
<meta charset="UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>POSSE 管理画面ダッシュボード</title>
<!-- スタイルシート読み込み -->
<link rel="stylesheet" href="../assets/styles/common.css">
<link rel="stylesheet" href="../admin.css">
<!-- Google Fonts読み込み -->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;700&family=Plus+Jakarta+Sans:wght@400;700&display=swap" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
</head>

<body>
<div class="wrapper">
    <main>
    <div class="container">
        <h1 class="mb-4">問題作成</h1>
        <form class="question-form" method="POST" enctype="multipart/form-data">
        <div class="mb-4">
            <label for="question" class="form-label">問題文:</label>
            <!-- name属性でsqlとつなげる -->
            <input type="text" name="content" id="question" class="form-control required" placeholder="問題文を入力してください" required/>
        </div>
        <div class="mb-4">
            <label class="form-label">選択肢:</label>
            <div class="form-choices-container">
                <input type="text" name="choices[]" class="required form-control mb-2" placeholder="選択肢1を入力してください" required>
                <input type="text" name="choices[]" class="required form-control mb-2" placeholder="選択肢2を入力してください"required>
                <input type="text" name="choices[]" class="required form-control mb-2" placeholder="選択肢3を入力してください"required>
            </div>
        </div>
        <div class="mb-4">
            <label class="form-label">正解の選択肢</label>
            <div class="form-check-container">
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="correctChoice" id="correctChoice1" checked value="1"required>
                    <label class="form-check-label" for="correctChoice1">
                    選択肢1
                    </label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="correctChoice" id="correctChoice2" value="2"required>
                    <label class="form-check-label" for="correctChoice2">
                    選択肢2
                    </label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="correctChoice" id="correctChoice2" value="3"required>
                    <label class="form-check-label" for="correctChoice2">
                    選択肢3
                    </label>
                </div>
            </div>
        </div>
        <div class="mb-4">
            <label for="question" class="form-label">問題の画像</label>
            <input type="file" name="image file" id="image" class="form-control required" required/>
        </div>
        <div class="mb-4">
            <label for="question" class="form-label">補足:</label>
            <input type="text" name="supplement" id="supplement" class="form-control" placeholder="補足を入力してください" />
        </div>
        <button type="submit" disabled class="btn submit">作成</button>
        </form>
    </div>
    </main>
</div>
<script>
    const submitButton = document.querySelector('.btn.submit')
    const inputDoms = Array.from(document.querySelectorAll('.required'))
    inputDoms.forEach(inputDom => {
    inputDom.addEventListener('input', event => {
        const isFilled = inputDoms.filter(d => d.value).length === inputDoms.length
        submitButton.disabled = !isFilled
    })
    })
</script>
</body>

</html>